...
시스템은 기획부터 구현까지 논리적이고 세분화된 4단계 트리 계층 구조를 가집니다.

### 2-1. 도메인 계층 구조 (Hierarchy)
**`요구사항(Requirement) ➔ 화면(Screen) ➔ 영역(Area) ➔ 기능(Function) ⚡(Trigger)➔ AI작업(AiTask)`**

- **Requirement (요구사항)**: 고객의 비즈니스적 요구 메인 단위 (예: "회원 관리 시스템 구축")
