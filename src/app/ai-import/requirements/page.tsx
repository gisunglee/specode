...
import {
  ...
  FileSearch, ClipboardList, BookMarked,
} from "lucide-react";
  ...
  requirements: [
    {
      ...
      originalContent: "계약서/RFP 원문 근거",
      currentContent: "협의를 통해 구체화된 내용",
      detailSpec: "상세 명세 (마크다운 형식으로 작성)",
      priority: "HIGH",
      source: "RFP",
      userStories: [
        {
...
| 과업 정의 (definition) | ✅ 권장 | 과업 범위, 주요 목적, 처리 대상 |
...
| 원문 (originalContent) | ✅ 권장 | RFP/계약서의 원문 근거 |
| 현행화 (currentContent) | ✅ 권장 | 협의를 통해 구체화된 내용 |
| 상세 명세 (detailSpec) | ✅ 권장 | 마크다운 형식의 상세 설명 |
  ...
  "requirements": [
    {
      ...
      "currentContent": "이메일 인증 추가, SNS 연동 로그인 포함",
      "detailSpec": "### 기능 설명\\n이메일 기반 회원 가입 기능을 제공합니다.\\n\\n### 처리 흐름\\n1. 이메일/비밀번호 입력\\n2. 이메일 인증\\n3. 가입 완료",
      "priority": "HIGH",
      "source": "RFP",
      "userStories": [
        {
...
interface ParsedData {
  task: { systemId?: string; name: string; definition?: string; outputInfo?: string };
  requirements?: ParsedReq[];
}

interface ImportResult {
  task: { systemId: string; name: string; isNew: boolean };
  summary: {
    requirements: number;
    userStories: number;
    new: { task: number; requirements: number; userStories: number };
    updated: { task: number; requirements: number; userStories: number };
  };
}
...
function PreviewTree({ data }: { data: ParsedData }) {
  const countNew = { reqs: 0, stories: 0 };
  const countUpd = { reqs: 0, stories: 0 };
  (data.requirements ?? []).forEach(r => {
    r.systemId ? countUpd.reqs++ : countNew.reqs++;
    (r.userStories ?? []).forEach(s => { s.systemId ? countUpd.stories++ : countNew.stories++; });
  });
  ...
  return (
    <div className="text-xs space-y-2">
      ...
      <div className="flex items-center gap-1.5 font-semibold text-foreground">
        ...
        {data.task.name}
        <Badge isNew={!data.task.systemId} />
        {data.task.systemId && <span className="font-mono text-muted-foreground">{data.task.systemId}</span>}
      </div>

      {/* 요구사항 */}
      {(data.requirements ?? []).map((r, ri) => (
...
function CompletenessBar({ data }: { data: ParsedData }) {
  ...
  const checks = [
    { label: "과업 정의",       ok: !!data.task.definition?.trim() },
    { label: "과업 산출물",     ok: !!data.task.outputInfo?.trim() },
...
export default function AiImportRequirementsPage() {
  ...
  const { data: taskData } = useQuery({
    ...
    queryFn: () => apiFetch<{ data: TaskItem[] }>("/api/tasks?pageSize=200"),
  });
  ...
  const isJsonValid = parsed !== null && !!parsed.task?.name;
  ...
  const handleExport = async () => {
    ...
    try {
      const res = await apiFetch<{ data: object }>(`/api/requirements-import?taskId=${exportTaskId}`);
            ...
            <div className="rounded-lg border border-border bg-card p-5">
              ...
              <ol className="space-y-2 text-sm text-muted-foreground">
                {[
                  ...
                  "Claude와 3단계로 설계합니다: ① 과업 → ② 요구사항 → ③ 사용자스토리. 누락 항목은 Claude가 다시 물어봅니다.",
                  "중간에 '요약해줘'로 현황을 확인하고, 완료 후 \"JSON 줘\" 요청 → Claude가 엄격 검토 후 출력합니다.",
                  "기존 데이터 수정: ② 탭에서 과업 내보내기 → JSON 복사 → Claude에 붙여넣고 수정 요청 → JSON 받기.",
              ...
              <div className="p-4 overflow-x-auto">
                <table className="w-full text-xs">
                  ...
                  <tbody className="divide-y divide-border/50">
                    {[
                      ["과업",         "systemId", "T-NNNNN",  "신규 생성 → 기존 수정"],
                      ["요구사항",     "systemId", "RQ-NNNNN", "신규 생성 → 기존 수정"],
                      ["사용자스토리", "systemId", "US-NNNNN", "신규 생성 → 기존 수정"],
        ...
        {tab === "import" && (
          <div className="max-w-5xl mx-auto px-6 py-6 space-y-6">
            ...
            <div className="rounded-lg border border-border bg-card p-4">
              ...
              <p className="text-xs text-muted-foreground mb-3">과업을 선택하면 systemId 포함 JSON을 클립보드에 복사합니다. Claude에 붙여넣고 수정을 요청하세요.</p>
              <div className="flex gap-2">
                <Select value={exportTaskId} onValueChange={setExportTaskId}>
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      ...
                      <div className="flex gap-4 text-green-700">
                        ...
                        <span><b>{importResult.summary.new.userStories}</b> 사용자스토리</span>
                      </div>
                    </div>
                    <div>
                      ...
                      <div className="flex gap-4 text-amber-700">
                        ...
                        <span><b>{importResult.summary.updated.userStories}</b> 사용자스토리</span>
                      </div>
                    </div>
                  </div>
                  <p className="text-xs text-green-600 mt-2">
                    과업 <strong>{importResult.task.systemId}</strong> {importResult.task.name}
                    {importResult.task.isNew ? " 이(가) 새로 생성되었습니다." : " 이(가) 업데이트되었습니다."}
                  </p>
                </div>
